...
import { FileText, Search, Filter, X, ChevronLeft, ChevronRight, Calendar } from 'lucide-react';
             ...
             <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                        <FileText className="w-6 h-6 text-indigo-600"/> All Expenditure Records
                    </h1>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div>
                         <button 
                            ...
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
                        >
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Attachment</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                            {loading ? (
                                ...
                                <tr><td colSpan="6" className="px-6 py-12 text-center text-gray-500">No records found matching your filters.</td></tr>
                            ) : items.map((item) => (
                                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        {item.attachment_url && (
                                            <a href={`${import.meta.env.VITE_API_URL}${item.attachment_url}`} target="_blank" className="text-indigo-600 hover:text-indigo-900">
                                                View
                                            </a>
                                        )}
                                    </td>
